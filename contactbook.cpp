                        ...
                        case 2:
                            if (phones.empty()) {
                                ...
                                if (removeIndex < 1 || removeIndex > phones.size()) {
                                    ...
                                    // Используем метод remove_phone (нужно добавить в класс Contact)
...
void Contactbook::sort_by_phones() {
    ...
    contactbook.sort([](Contact& a, Contact& b) {
        ...
        /*if (!phones_a.empty() && !phones_b.empty()) {
            return phones_a.front() < phones_b.front();
        }
        // Contacts with phones come before contacts without phones
        else if (!phones_a.empty() && phones_b.empty()) {
            return true;
        }
        else if (phones_a.empty() && !phones_b.empty()) {
            return false;
        }
        // Both have no phones - consider them equal
        else {
            return false;
        }*/
    });
...
bool Contactbook::load_from_file(const string& filename) {
    ...
    while (getline(file, line)) {
        if (line == "===CONTACT===") {
            ...
            // Read phone numbers until we hit ===END===
            string phone;
